...
export function render(state) {
  ...
  return `<section class="page-shell" aria-labelledby="view-title">
    ...
    <div class="plan-grid"><div class="card"><h2>${escapeHTML(t("plan.steps"))}</h2><ol class="timeline">
      ${timelineRow("1", t("plan.prep"), t("plan.prepCount", { done: state.prep.length, total: 3 }), "prep")}
    ...
    <div class="card attestation-card"><h2>${escapeHTML(t("plan.certificate"))}</h2>${complete ? `<p class="small subtle">${escapeHTML(t("plan.certificateSoon"))}</p>` : `<button class="button button-secondary" type="button" disabled>${escapeHTML(t("plan.certificate"))}</button><p class="small subtle">${escapeHTML(t("plan.certificateLocked"))}</p>`}</div>
