...
function courseAdmin(ui) {
  ...
  const errors = Array.isArray(ui.courseErrors) && ui.courseErrors.length ? `<div class="course-errors" role="alert"><h3>${escapeHTML(t("course.errorsTitle"))}</h3><ul>${ui.courseErrors.map((error) => `<li>${escapeHTML(t(`course.err.${error.code}`, { path: error.path, max: error.params?.max }))}</li>`).join("")}</ul></div>` : "";
